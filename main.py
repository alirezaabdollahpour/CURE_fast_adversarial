...
def get_args():
    ...
    parser.add_argument('--attack', default='multigrad', type=str, choices=['zerograd', 'fgsm', 'pgd'])
...
def main():
    ...
    results_csv = f'train_FGSM_h_{args.h}_betta_{args.betta}_lambda_{args.lambda_}_epochs_{args.epochs}_lr_{args.lr_max}_lr_schedule_{args.lr_schedule}_{args.delta}_loss_clean_g2_g3_direction_{args.attack}.csv'
    ...
    accuracy_df = pd.DataFrame(columns=['epoch','loss_train_FGSM','loss_train_clean','ACC_train','ACC_Clean_train','ACC_test','Curvature','grad_norm','test_loss_clean'])
    for epoch in range(args.epochs):
        ...
        for i, (X, y) in tqdm(enumerate(train_loader)):
            ...
            if args.attack == 'fgsm':
                ...
                delta = zero_grad(model, X, y, epsilon, args.fgsm_alpha * epsilon, args.zero_qval, args.zero_iters, args.delta_init)
            ...
            with torch.cuda.amp.autocast():
                ...
                if args.delta == 'linf':
                    ...
                    loss = loss + 100*regularizer + 10*gradient_norm
        ...
        if args.early_stop:
            ...
            test_robust_acc = 0
            test_n = 0
            delta = attack_pgd_Alireza(model, X, y, epsilon, pgd_alpha, args.test_iters, args.test_restarts, 'l_inf', early_stop=args.eval, fgsm_init=None)
            delta = delta.detach()
            ...
            print("="*60)
            # if test_robust_acc - prev_robust_acc < -0.2:
            #     break
            # prev_robust_acc = test_robust_acc
            # best_state_dict = copy.deepcopy(model.state_dict())
            # test_acc = test_robust_acc
        ...
        ###########################################################################
        accuracy_df = accuracy_df.append({'epoch': epoch, 'loss_train_FGSM': train_loss/train_n ,'loss_train_clean': train_loss_clean/train_n,'ACC_train':(train_acc/train_n)*100,'ACC_Clean_train':(train_acc_clean/train_n)*100,'ACC_test':(test_acc/test_n)*100, 'Curvature':curvature, 'grad_norm':gradient_norm,'test_loss_clean':test_loss_clean/test_n}, ignore_index=True)
